...
test('PostgreSQL store conforms for accounts, reservations, credentials, and rate limits',{skip:!connectionString},async()=>{
 ...
 try{
  ...
  const concurrentRequest=`parallel_${suffix}`,[parallelA,parallelB]=await Promise.all([store.reserve(session.account.id,concurrentRequest,25),store.reserve(session.account.id,concurrentRequest,25)]);assert.equal(parallelA.id,parallelB.id);assert.equal((await store.release(session.account.id,parallelA.id)).status,'released');
  const purchase={eventId:`event_${suffix}`,accountId:session.account.id,sessionId:`checkout_${suffix}`,packageId:'test',amountPaid:900,currency:'USD',credits:50};await Promise.all([store.grantPurchase(purchase),store.grantPurchase(purchase)]);assert.equal((await store.purchases(session.account.id)).filter(item=>item.sessionId===purchase.sessionId).length,1);
