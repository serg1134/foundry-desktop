...
export class PreviewService{
  ...
  async build(project:ProjectRecord,persistentStorage=false):Promise<PreviewResult>{
    ...
    const bootstrap=persistentStorage?'':`<script>${storageShim}</script>`,allowNetwork=this.networkAllowed?await this.networkAllowed(project):false;
    return{html:`<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="${previewCsp(allowNetwork)}"><style>html,body,#root{min-height:100%;margin:0}${css}</style></head><body><div id="root"></div>${bootstrap}<script>${diagnosticBridge}</script><script>${selectionBridge}</script><script>${safeJs}</script></body></html>`,warnings:result.warnings.map(item=>item.text)};
  }
}

export function previewCsp(networkAllowed=false):string{return`default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; font-src data:${networkAllowed?'; connect-src https: wss:':''}`}
