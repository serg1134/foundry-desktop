...
export type ProjectConfig={
  ...
  capabilities:{openTextFile:boolean;saveTextFile:boolean;folderRead:boolean;database:boolean;network:boolean;clipboardRead:boolean;clipboardWrite:boolean;notifications:boolean;tray:boolean;globalShortcuts:boolean;menus:boolean;deepLinks:boolean};
...
export class ProjectConfigService{
  ...
  async reconcileNativeCapabilities(project:ProjectRecord):Promise<ProjectConfig>{
    const config=await this.get(project),required=await detectNativeCapabilities(project.root),capabilities={...config.capabilities},previouslyGenerated=await this.generatedCapabilities(project),nextGenerated=new Set<keyof ProjectConfig['capabilities']>();let changed=false;
    for(const key of Object.keys(capabilities) as (keyof ProjectConfig['capabilities'])[]){
      if(required[key]){nextGenerated.add(key);if(!capabilities[key]){capabilities[key]=true;changed=true}}
      else if(previouslyGenerated.has(key)&&capabilities[key]){capabilities[key]=false;changed=true}
    }
    await mkdir(join(project.root,'.foundry'),{recursive:true});
    await writeFile(this.generatedCapabilitiesPath(project),JSON.stringify([...nextGenerated].sort()),'utf8');
  ...
  async setIconPng(project:ProjectRecord,bytes:Buffer):Promise<ProjectConfig>{
    if(bytes.length<8||bytes.length>15_000_000||!bytes.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])))throw new Error('The generated icon is not a valid PNG image.');
    const folder=join(project.root,'.foundry'),target=join(folder,'app-icon.png');await mkdir(folder,{recursive:true});await writeFile(target,bytes);const config=await this.get(project);return this.save(project,{...config,icon:'.foundry/app-icon.png'});
  }
  async iconDataUrl(project:ProjectRecord):Promise<string|null>{
    const config=await this.get(project);if(!config.icon)return null;const bytes=await readFile(join(project.root,...config.icon.split('/')));if(bytes.length>15_000_000)throw new Error('The application icon is too large.');return`data:image/${config.icon.endsWith('.ico')?'x-icon':'png'};base64,${bytes.toString('base64')}`;
  }
  ...
  private generatedCapabilitiesPath(project:ProjectRecord):string{return join(project.root,'.foundry','generated-capabilities.json')}
  private async generatedCapabilities(project:ProjectRecord):Promise<Set<keyof ProjectConfig['capabilities']>>{try{const value=JSON.parse(await readFile(this.generatedCapabilitiesPath(project),'utf8'));if(!Array.isArray(value))return new Set();const allowed=new Set(Object.keys(this.defaults(project).capabilities));return new Set(value.filter((item):item is keyof ProjectConfig['capabilities']=>typeof item==='string'&&allowed.has(item)))}catch(error){if((error as NodeJS.ErrnoException).code==='ENOENT'||error instanceof SyntaxError)return new Set();throw error}}
  private defaults(project:ProjectRecord):ProjectConfig{const id=safeId(project.name);return{displayName:project.name,description:`Desktop app created with Foundry`,version:'0.1.0',publisher:'',appId:`com.foundry.${id}`,window:{width:1200,height:800,minWidth:720,minHeight:480,resizable:true,maximizable:true},installer:{oneClick:false,allowDirectorySelection:true,desktopShortcut:true,startMenuShortcut:true},capabilities:{openTextFile:false,saveTextFile:false,folderRead:false,database:false,network:false,clipboardRead:false,clipboardWrite:false,notifications:false,tray:false,globalShortcuts:false,menus:false,deepLinks:false},ai:{mode:'none',provider:'openai',model:'gpt-5.6-sol',monthlyBudget:1000,requestsPerMinute:10}}}
...
async function detectNativeCapabilities(root:string):Promise<Partial<ProjectConfig['capabilities']>>{
  ...
  for(const entry of files){
    ...
    const parent='parentPath'in entry&&typeof entry.parentPath==='string'?entry.parentPath:join(root,'src'),source=await readFile(join(parent,entry.name),'utf8'),bridge=bridgeReferencePattern(source);
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)openTextFile\\s*\\(`).test(source))required.openTextFile=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)saveTextFile\\s*\\(`).test(source))required.saveTextFile=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)chooseFolder\\s*\\(`).test(source))required.folderRead=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)database\\.`).test(source))required.database=true;
    if(/\b(?:fetch|XMLHttpRequest|WebSocket|EventSource)\s*(?:\(|\.)/.test(source))required.network=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)readClipboardText\\s*\\(`).test(source))required.clipboardRead=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)writeClipboardText\\s*\\(`).test(source))required.clipboardWrite=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)showNotification\\s*\\(`).test(source))required.notifications=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)tray\\.`).test(source))required.tray=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)shortcuts\\.`).test(source))required.globalShortcuts=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)menus\\.`).test(source))required.menus=true;
    if(new RegExp(`${bridge}(?:\\?\\.|\\.)deepLinks\\.`).test(source))required.deepLinks=true;
  }
  return required;
}

function bridgeReferencePattern(source:string):string{
  const aliases=new Set<string>(['foundryDesktop']);let changed=true;
  while(changed){changed=false;for(const match of source.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:window\s*\.\s*)?([A-Za-z_$][\w$]*)\b/g)){if(aliases.has(match[2])&&!aliases.has(match[1])){aliases.add(match[1]);changed=true}}}
  const helpers=new Set<string>();for(const match of source.matchAll(/\b(?:function\s+([A-Za-z_$][\w$]*)\s*\([^)]*\)|(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*\([^)]*\)\s*=>)\s*\{[\s\S]{0,1200}?\breturn\s+(?:window\s*\.\s*)?foundryDesktop\b/g))helpers.add(match[1]||match[2]);
  const escapedAliases=[...aliases].map(value=>value.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).join('|'),helperCalls=[...helpers].map(value=>`${value.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s*\\(\\s*\\)`).join('|');
  return`(?:(?:window\\s*\\.\\s*)?(?:${escapedAliases})${helperCalls?`|(?:${helperCalls})`:''})\\s*!?`;
}
