...
export class ProjectConfigService{
  ...
  private defaults(project:ProjectRecord):ProjectConfig{const id=safeId(project.name);return{displayName:project.name,description:`Desktop app created with Foundry`,version:'0.1.0',publisher:'',appId:`com.foundry.${id}`,window:{width:1200,height:800,minWidth:720,minHeight:480,resizable:true,maximizable:true},installer:{oneClick:false,allowDirectorySelection:true,desktopShortcut:true,startMenuShortcut:true},capabilities:{openTextFile:true,saveTextFile:true,folderRead:false,database:false,network:false,clipboardRead:false,clipboardWrite:false,notifications:false,tray:false,globalShortcuts:false,menus:false,deepLinks:false},ai:{mode:'none',provider:'openai',model:'gpt-5.6-sol',monthlyBudget:1000,requestsPerMinute:10}}}
...
async function detectNativeCapabilities(root:string):Promise<Partial<ProjectConfig['capabilities']>>{
  ...
  for(const entry of files){
    ...
    if(/foundryDesktop(?:\?\.|\.)menus\./.test(source))required.menus=true;
    if(/foundryDesktop(?:\?\.|\.)deepLinks\./.test(source))required.deepLinks=true;
  }
  return required;
}
