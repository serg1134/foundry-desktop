...
function registerWorkspaceHandlers():void{
  ...
  ipcMain.handle('benchmarks:run',async(event,id)=>{assertTrusted(event);if(typeof id!=='string')throw new Error('A benchmark id is required.');const benchmark=benchmarks.case(id),credential=await settings.credential(),started=Date.now();await mkdir(benchmarkRoot,{recursive:true});const project=await benchmarkWorkspace.createProject(benchmarkRoot,`Benchmark ${benchmark.id} ${Date.now()}`);await checkpoints.ensure(project);let filesChanged=0;try{const initial=await agent.run(project,benchmark.prompt,credential,update=>event.sender.send('agent:event',update)),workflow=benchmark.workflow??initial.workflow,firstVerification=await verifyProject(project,workflow),loop=await runBehaviorRepairLoop({request:benchmark.prompt,result:initial,workflow,verification:firstVerification,verify:()=>verifyProject(project,workflow),repair:(repairPrompt,attempt)=>{event.sender.send('agent:event',{type:'status',message:`Benchmark verification failed. Running targeted repair ${attempt} of ${MAX_BEHAVIOR_REPAIRS}…`});return agent.run(project,repairPrompt,credential,update=>event.sender.send('agent:event',update))},onAttempt:attempt=>event.sender.send('agent:event',{type:'verify',message:attempt.progress?`Repair ${attempt.attempt} improved the ${attempt.stage} checks.`:`Repair ${attempt.attempt} did not improve the ${attempt.stage} checks.`})});filesChanged=loop.result.filesChanged.length;const checksPassed=loop.verification.checks.filter(check=>check.passed).length;return benchmarks.record({id:randomUUID(),benchmarkId:id,projectId:project.id,passed:loop.verification.passed,durationMs:Date.now()-started,checksPassed,checksTotal:loop.verification.checks.length,filesChanged,completedAt:new Date().toISOString(),repairAttempts:loop.attempts.length,repaired:!firstVerification.passed&&loop.verification.passed,repairStages:loop.attempts.map(attempt=>attempt.stage),...(!loop.verification.passed?{failure:failedVerificationChecks(loop.verification).map(check=>check.detail).join(' ')}:{})})}catch(error){return benchmarks.record({id:randomUUID(),benchmarkId:id,passed:false,durationMs:Date.now()-started,checksPassed:0,checksTotal:0,filesChanged,completedAt:new Date().toISOString(),failure:error instanceof Error?error.message:String(error)})}});
