import { app, BrowserWindow, clipboard, dialog, globalShortcut, ipcMain, Menu, nativeImage, net, Notification, session, shell, Tray, type IpcMainInvokeEvent } from 'electron';
...
import { configurePackagedEsbuild, previewDependencyRoot, PreviewService } from './preview';
...
import { failedVerificationChecks, MAX_BEHAVIOR_REPAIRS, MAX_VISUAL_REPAIRS, runBehaviorRepairLoop, runReliabilityStage, runVisualRepairLoop } from './reliability';
...
const verificationWindowIds=new Set<number>();
const runtimeTrays=new Map<string,Tray>();
const runtimeShortcuts=new Map<string,Set<string>>();
const runtimeMenus=new Set<string>();
const runtimeDeepLinks=new Set<string>();
const runtimeNotifications=new Set<string>();
...
function assertTrusted(event:IpcMainInvokeEvent):void{
  ...
  const trusted=(foundryWindow&&!foundryWindow.isDestroyed()&&foundryWindow.webContents.id===senderId)||[...runtimeWindows.values()].some(window=>!window.isDestroyed()&&window.webContents.id===senderId);
...
async function requireRuntimeCapability(event:IpcMainInvokeEvent,capability:keyof ProjectConfig['capabilities']):Promise<ProjectRecord>{
  ...
  const project=await projectById(entry[0]),config=await projectConfigs.reconcileNativeCapabilities(project);if(!config.capabilities[capability])throw new Error(`This app is not allowed to use ${capability}. Enable it in App settings.`);return project;
}

async function runtimeDatabase(project:ProjectRecord,senderId?:number):Promise<CapabilityDatabase>{
  if(senderId!==undefined&&verificationWindowIds.has(senderId)){
    let database=verificationDatabases.get(senderId);if(database)return database;
    database=await prepareCapabilityDatabase(join(project.root,'.foundry','verification-data',String(senderId),'app.sqlite'));verificationDatabases.set(senderId,database);return database;
  }
  let database=runtimeDatabases.get(project.id);if(database)return database;database=await prepareCapabilityDatabase(join(project.root,'.foundry','runtime-data','app.sqlite'));runtimeDatabases.set(project.id,database);return database
}
async function clearVerificationDatabase(project:ProjectRecord,senderId:number):Promise<void>{verificationDatabases.get(senderId)?.close();verificationDatabases.delete(senderId);verificationWindowIds.delete(senderId);await rm(join(project.root,'.foundry','verification-data',String(senderId)),{recursive:true,force:true,maxRetries:6,retryDelay:100})}
...
function clearRuntimeCapabilities(projectId:string):void{clearRuntimeShortcuts(projectId);runtimeTrays.get(projectId)?.destroy();runtimeTrays.delete(projectId);runtimeDatabases.get(projectId)?.close();runtimeDatabases.delete(projectId);runtimeMenus.delete(projectId);runtimeDeepLinks.delete(projectId);runtimeNotifications.delete(projectId)}
...
const starterIconColors:Record<string,{start:string;end:string}>={ember:{start:'#ff6a3d',end:'#ff3d71'},ocean:{start:'#1677ff',end:'#55c2ff'},mint:{start:'#18a87b',end:'#75e3b5'},violet:{start:'#7c4dff',end:'#c267ff'}};
async function starterIconPng(name:string,style:unknown='ember'):Promise<Buffer>{const colors=starterIconColors[typeof style==='string'?style:'']??starterIconColors.ember,initial=(name.match(/[A-Za-z0-9]/)?.[0]??'A').toUpperCase(),svg=`<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${colors.start}"/><stop offset="1" stop-color="${colors.end}"/></linearGradient></defs><rect width="1024" height="1024" rx="230" fill="url(#g)"/><circle cx="512" cy="475" r="315" fill="#fff" fill-opacity=".12"/><text x="512" y="660" text-anchor="middle" font-family="Arial,sans-serif" font-weight="700" font-size="500" fill="#fff">${initial}</text></svg>`,image=nativeImage.createFromDataURL(`data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}`);if(!image.isEmpty()){const png=image.resize({width:1024,height:1024,quality:'best'}).toPNG();if(png.length)return png}const fallback=await readFile(appIconPath());if(!fallback.length)throw new Error('Could not create the starter icon.');return fallback}
function pngFromDataUrl(value:unknown):Buffer{if(typeof value!=='string'||!/^data:image\/png;base64,[A-Za-z0-9+/=]+$/.test(value))throw new Error('A valid generated PNG is required.');const bytes=Buffer.from(value.slice(value.indexOf(',')+1),'base64');if(bytes.length>15_000_000)throw new Error('The generated icon is too large.');return bytes}
async function generateIconCandidate(config:ProjectConfig,prompt:unknown):Promise<string>{const direction=typeof prompt==='string'?prompt.trim():'';if(!direction||direction.length>600)throw new Error('Describe the icon in 600 characters or fewer.');const credential=await settings.credential(),requestId=`icon_${randomUUID().replace(/-/g,'')}`;let imageBase64:string;if(credential.mode==='hosted'){const client=new CloudClient(credential.gatewayUrl,(input,init)=>net.fetch(input,init)),result=await client.generateIcon(credential.apiKey,{prompt:direction,appName:config.displayName,description:config.description,requestId});imageBase64=result.imageBase64}else{if(credential.provider!=='openai')throw new Error('Select OpenAI in Builder settings, or use Foundry Cloud, to generate an icon.');const response=await net.fetch('https://api.openai.com/v1/images/generations',{method:'POST',headers:{Authorization:`Bearer ${credential.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-image-2',n:1,size:'1024x1024',quality:'medium',background:'transparent',output_format:'png',prompt:`Create a polished desktop application icon for “${config.displayName}”. Purpose: ${config.description}. Direction: ${direction}. Single centered symbol, simple silhouette, high contrast, premium native desktop aesthetic, readable at 16 pixels, no words, no letters, no mockup, no border, transparent background.`})}),body=await response.json() as {data?:{b64_json?:string}[];error?:{message?:string}};if(!response.ok)throw new Error(body.error?.message??'OpenAI could not generate the icon.');imageBase64=body.data?.[0]?.b64_json??''}const bytes=pngFromDataUrl(`data:image/png;base64,${imageBase64}`);return`data:image/png;base64,${bytes.toString('base64')}`}
...
function registerWorkspaceHandlers():void{
  ...
  ipcMain.handle('workspace:create',async(event,name:unknown,template:unknown)=>{
    ...
    const project=await workspace.createProject(result.filePaths[0],name,template as TemplateId);await checkpoints.ensure(project);await projectConfigs.setIconPng(project,await starterIconPng(project.name));return project;
  });
  ...
  ipcMain.handle('desktop-database:get',async(event,value)=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');return(await runtimeDatabase(project,event.sender.id)).get(value?.namespace,value?.key)});
  ipcMain.handle('desktop-database:set',async(event,value)=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');return(await runtimeDatabase(project,event.sender.id)).set(value?.namespace,value?.key,value?.value)});
  ipcMain.handle('desktop-database:delete',async(event,value)=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');return(await runtimeDatabase(project,event.sender.id)).delete(value?.namespace,value?.key)});
  ipcMain.handle('desktop-database:list',async(event,value)=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');return(await runtimeDatabase(project,event.sender.id)).list(value?.namespace)});
  ipcMain.handle('desktop-database:backup',async event=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');(await runtimeDatabase(project,event.sender.id)).backup();return true});
  ipcMain.handle('desktop-database:restore-latest',async event=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'database');if(verificationWindowIds.has(event.sender.id))throw new Error('Database restore is unavailable during isolated verification.');const answer=await dialog.showMessageBox({type:'warning',buttons:['Cancel','Restore backup'],defaultId:0,cancelId:0,title:'Restore app data?',message:`Restore the latest backup for ${project.name}?`,detail:'The current database will be preserved as a quarantined recovery file. Changes made after the backup will be replaced.'});if(answer.response!==1)return false;return(await runtimeDatabase(project,event.sender.id)).restoreLatest()});
  ipcMain.handle('desktop:clipboard-read',async event=>{assertTrusted(event);await requireRuntimeCapability(event,'clipboardRead');return clipboard.readText().slice(0,100_000)});
  ...
  ipcMain.handle('desktop:menu-configure',async(event,value)=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'menus'),window=runtimeWindows.get(project.id),items=Array.isArray(value?.items)?value.items:[];if(!window||window.isDestroyed())throw new Error('The project window is not available.');if(!items.length||items.length>30)throw new Error('Menu configuration is invalid.');const normalized=items.map(item=>{const id=String(item?.id??''),label=String(item?.label??'').trim(),accelerator=item?.accelerator==null?undefined:String(item.accelerator).trim();if(!/^[A-Za-z0-9._:-]{1,80}$/.test(id)||!label||label.length>80||(accelerator&&!/^[A-Za-z0-9+_-]{1,60}$/.test(accelerator)))throw new Error('Menu item identifiers, labels, or shortcuts are invalid.');return{label,...(accelerator?{accelerator}:{}),click:()=>window.webContents.send('foundry-desktop:menu-action',id)}});window.setMenu(Menu.buildFromTemplate([{label:project.name,submenu:normalized}]));runtimeMenus.add(project.id);return true});
  ipcMain.handle('desktop:deep-link-initial',async event=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'deepLinks');runtimeDeepLinks.add(project.id);return null});
  ipcMain.handle('desktop:deep-link-ready',async event=>{assertTrusted(event);const project=await requireRuntimeCapability(event,'deepLinks');runtimeDeepLinks.add(project.id);return true});
  ...
  ipcMain.handle('project-config:icon-data',async(event,id)=>{assertTrusted(event);return projectConfigs.iconDataUrl(await projectById(id))});
  ipcMain.handle('project-config:generate-icon',async(event,id,prompt)=>{assertTrusted(event);const project=await projectById(id);return generateIconCandidate(await projectConfigs.get(project),prompt)});
  ipcMain.handle('project-config:apply-icon',async(event,id,dataUrl)=>{assertTrusted(event);const project=await projectById(id),saved=await projectConfigs.setIconPng(project,pngFromDataUrl(dataUrl));await applyRuntimeConfig(project);return saved});
  ipcMain.handle('project-config:starter-icon',async(event,id,style)=>{assertTrusted(event);const project=await projectById(id),config=await projectConfigs.get(project),saved=await projectConfigs.setIconPng(project,await starterIconPng(config.displayName,style));await applyRuntimeConfig(project);return saved});
...
async function runAgentRequest(event:IpcMainInvokeEvent,id:unknown,prompt:unknown,attachments:unknown){
 ...
 try{
  ...
  const abortTimedOutStage=(stage:string)=>{send({type:'status',message:`${stage} took too long and was stopped safely.`});controller.abort()};
  send({type:'verify',message:'Launching an isolated verification window…'});const firstVerification=await runReliabilityStage('initial behavior verification',()=>verifyProject(project,workflow),undefined,abortTimedOutStage),loop=await runBehaviorRepairLoop({request:prompt,result:initial,workflow,verification:firstVerification,verify:()=>verifyProject(project,workflow),repair:(repairPrompt,attempt)=>{send({type:'status',message:`Verification diagnosed a problem. Running targeted repair ${attempt} of ${MAX_BEHAVIOR_REPAIRS}…`});return agent.run(project,repairPrompt,credential,send,controller.signal)},onAttempt:attempt=>send({type:'verify',message:attempt.progress?`Repair ${attempt.attempt} improved the ${attempt.stage} checks.`:`Repair ${attempt.attempt} did not improve the ${attempt.stage} checks.`}),onTimeout:abortTimedOutStage});let{result,verification}=loop;
...
async function verifiedWorkflow(project:ProjectRecord):Promise<WorkflowStep[]>{
  try{const value=JSON.parse(await readFile(join(project.root,'.foundry','verified-workflow.json'),'utf8')) as unknown;if(!Array.isArray(value))return[];return value.filter((step):step is WorkflowStep=>Boolean(step&&typeof step==='object'&&typeof (step as WorkflowStep).action==='string'&&typeof (step as WorkflowStep).target==='string'&&typeof (step as WorkflowStep).value==='string')).slice(0,10)}catch{return[]}
}
...
async function verifyProject(project:ProjectRecord,workflow:WorkflowStep[]=[]):Promise<VerificationResult>{
  ...
  const runtimeConfig=await projectConfigs.reconcileNativeCapabilities(project);
  const errors:string[]=[],verificationWindow=new BrowserWindow({show:false,width:1024,height:720,webPreferences:{preload:join(currentDirectory,'../preload/runtime.cjs'),sandbox:true,contextIsolation:true,nodeIntegration:false,offscreen:true,backgroundThrottling:false}});
  runtimeWindows.set(project.id,verificationWindow);verificationWindowIds.add(verificationWindow.webContents.id);
  try{
    ...
    if(workflow.length){
      ...
      for(const[index,step]of workflow.entries())if(step.action==='assert_clipboard'){
        const actual=clipboard.readText(),passed=actual===step.value;workflowEvidence[index]={passed,detail:passed?`Confirmed the OS clipboard contains “${step.value}”.`:`Expected OS clipboard text “${step.value}”, but found “${actual.slice(0,120)}”.`};
      }
      if(workflow.some(step=>step.action==='assert_persisted_text')){await loadRuntime(verificationWindow,project);await new Promise(resolve=>setTimeout(resolve,2000))}
      for(const[index,step]of workflow.entries())if(step.action==='assert_persisted_text'){
        const found=await verificationWindow.webContents.executeJavaScript(`(()=>{const normalize=value=>String(value||'').trim().toLowerCase().replace(/\\s+/g,' '),expected=${JSON.stringify(step.value.trim().toLowerCase().replace(/\s+/g,' '))};if(normalize(document.body.innerText).includes(expected))return true;return [...document.querySelectorAll('input,textarea')].some(element=>normalize(element.value).includes(expected))})()`) as boolean;
    ...
    if(runtimeConfig.capabilities.menus)checks.push({name:'Native menu',passed:runtimeMenus.has(project.id),detail:runtimeMenus.has(project.id)?'Configured a validated native application menu.':'Native menus are enabled, but the app did not configure one during startup or the verified workflow.'});
    if(runtimeConfig.capabilities.deepLinks)checks.push({name:'Deep links',passed:runtimeDeepLinks.has(project.id),detail:runtimeDeepLinks.has(project.id)?'Registered a renderer handler for cold-start and running-app links.':'Deep links are enabled, but the app did not register a link handler during startup or the verified workflow.'});
    if(runtimeConfig.capabilities.database){let databaseError='';try{await verificationWindow.webContents.executeJavaScript(`window.foundryDesktop.database.list('__foundry_verification__')`)}catch(error){databaseError=error instanceof Error?error.message:String(error)}const databaseOpened=verificationDatabases.has(verificationWindow.webContents.id);checks.push({name:'Local database',passed:databaseOpened&&!databaseError,detail:databaseOpened&&!databaseError?'Opened the isolated SQLite database through the typed bridge.':databaseError||'The isolated SQLite database did not open through the typed bridge.'})}
    if(runtimeConfig.capabilities.notifications)checks.push({name:'Desktop notification',passed:runtimeNotifications.has(project.id),detail:runtimeNotifications.has(project.id)?'Displayed a validated desktop notification without a runtime error.':'Notifications are enabled, but the app did not show one during startup or the verified workflow.'});
    if(runtimeConfig.capabilities.tray)checks.push({name:'System tray',passed:runtimeTrays.has(project.id),detail:runtimeTrays.has(project.id)?'Created a validated tray icon and menu.':'System tray access is enabled, but the app did not configure its tray behavior during startup or the verified workflow.'});
    if(runtimeConfig.capabilities.globalShortcuts){const count=runtimeShortcuts.get(project.id)?.size??0;checks.push({name:'Global shortcuts',passed:count>0,detail:count>0?`Registered ${count} global keyboard shortcut${count===1?'':'s'} and scheduled cleanup.`:'Global shortcuts are enabled, but the app did not register one during startup or the verified workflow.'})}
  ...
  }catch(error){checks.push({name:'Runtime',passed:false,detail:error instanceof Error?error.message:String(error)})}finally{const senderId=verificationWindow.webContents.id;if(runtimeWindows.get(project.id)===verificationWindow)runtimeWindows.delete(project.id);if(!verificationWindow.isDestroyed())verificationWindow.destroy();await clearVerificationDatabase(project,senderId);clearRuntimeCapabilities(project.id)}
...
async function openRuntime(project:ProjectRecord):Promise<void>{
  const config=await projectConfigs.reconcileNativeCapabilities(project);
  const existing=runtimeWindows.get(project.id);
  if(existing&&!existing.isDestroyed()){configureWindow(existing,config,project);await loadRuntime(existing,project);existing.show();existing.focus();return}
...
async function refreshRuntime(project:ProjectRecord):Promise<void>{
  ...
  configureWindow(window,await projectConfigs.get(project),project);await loadRuntime(window,project);
}

async function applyRuntimeConfig(project:ProjectRecord):Promise<void>{const window=runtimeWindows.get(project.id);if(window&&!window.isDestroyed())configureWindow(window,await projectConfigs.get(project),project)}
function configureWindow(window:BrowserWindow,config:ProjectConfig,project:ProjectRecord):void{window.setTitle(config.displayName);window.setMinimumSize(config.window.minWidth,config.window.minHeight);window.setResizable(config.window.resizable);window.setMaximizable(config.window.maximizable);window.setSize(config.window.width,config.window.height);if(config.icon){const iconPath=join(project.root,...config.icon.split('/'));window.setIcon(nativeImage.createFromPath(iconPath));if(process.platform==='win32')window.setAppDetails({appId:config.appId,appIconPath:iconPath,appIconIndex:0,relaunchCommand:process.execPath,relaunchDisplayName:config.displayName})}}
...
app.whenReady().then(()=>{
  ...
  if(app.isPackaged&&!configurePackagedEsbuild(process.resourcesPath))console.error('The packaged esbuild executable is unavailable.');
  ...
  preview=new PreviewService(previewDependencyRoot(app.getAppPath(),process.resourcesPath,app.isPackaged),async project=>(await projectConfigs.reconcileNativeCapabilities(project)).capabilities.network);
