...
function starterIconPng(name:string,style:unknown='ember'):Buffer{const colors=starterIconColors[typeof style==='string'?style:'']??starterIconColors.ember,initial=(name.match(/[A-Za-z0-9]/)?.[0]??'A').toUpperCase(),svg=`<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${colors.start}"/><stop offset="1" stop-color="${colors.end}"/></linearGradient></defs><rect width="1024" height="1024" rx="230" fill="url(#g)"/><circle cx="512" cy="475" r="315" fill="#fff" fill-opacity=".12"/><text x="512" y="660" text-anchor="middle" font-family="Arial,sans-serif" font-weight="700" font-size="500" fill="#fff">${initial}</text></svg>`,image=nativeImage.createFromDataURL(`data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}`);if(image.isEmpty())throw new Error('Could not create the starter icon.');return image.resize({width:1024,height:1024,quality:'best'}).toPNG()}
...
function registerWorkspaceHandlers():void{
  ...
  ipcMain.handle('workspace:create',async(event,name:unknown,template:unknown)=>{
    ...
    const project=await workspace.createProject(result.filePaths[0],name,template as TemplateId);await checkpoints.ensure(project);await projectConfigs.setIconPng(project,starterIconPng(project.name));return project;
  });
  ...
  ipcMain.handle('project-config:starter-icon',async(event,id,style)=>{assertTrusted(event);const project=await projectById(id),config=await projectConfigs.get(project),saved=await projectConfigs.setIconPng(project,starterIconPng(config.displayName,style));await applyRuntimeConfig(project);return saved});
...
async function verifyProject(project:ProjectRecord,workflow:WorkflowStep[]=[]):Promise<VerificationResult>{
  ...
  try{
    ...
    if(runtimeConfig.capabilities.database)checks.push({name:'Local database',passed:runtimeDatabases.has(project.id),detail:runtimeDatabases.has(project.id)?'Opened the isolated SQLite database through the typed bridge.':'Local database access is enabled, but the app did not exercise it during startup or the verified workflow.'});
