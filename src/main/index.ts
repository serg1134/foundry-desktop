...
async function verifyProject(project:ProjectRecord,workflow:WorkflowStep[]=[]):Promise<VerificationResult>{
  ...
  runtimeWindows.set(project.id,verificationWindow);
  try{
  ...
  }catch(error){checks.push({name:'Runtime',passed:false,detail:error instanceof Error?error.message:String(error)})}finally{if(runtimeWindows.get(project.id)===verificationWindow)runtimeWindows.delete(project.id);clearRuntimeCapabilities(project.id);if(!verificationWindow.isDestroyed())verificationWindow.destroy()}
...
app.whenReady().then(()=>{
  ...
  preview=new PreviewService(app.getAppPath(),async project=>(await projectConfigs.reconcileNativeCapabilities(project)).capabilities.network);
