...
export class BillingService{
 constructor(private readonly secretKey:string,private readonly webhookSecret:string,private readonly successUrl:string,private readonly cancelUrl:string,private readonly packages=creditPackages(),private readonly fetcher:FetchLike=fetch){}
 catalog():Omit<CreditPackage,'priceId'>[]{return this.packages.map(({priceId:_,...item})=>item)}
 configured():boolean{return Boolean(this.secretKey&&this.webhookSecret&&this.packages.every(item=>item.priceId))}
 async checkout(account:GatewayAccount,packageId:string):Promise<CheckoutSession>{const item=this.packages.find(entry=>entry.id===packageId);if(!item||!item.priceId)throw new Error('That credit package is not available.');if(!this.secretKey)throw new Error('Billing is temporarily unavailable.');const form=new URLSearchParams({'mode':'payment','success_url':this.successUrl,'cancel_url':this.cancelUrl,'client_reference_id':account.id,'customer_email':account.email,'line_items[0][price]':item.priceId,'line_items[0][quantity]':'1','metadata[account_id]':account.id,'metadata[package_id]':item.id,'metadata[credits]':String(item.credits)}),response=await this.fetcher('https://api.stripe.com/v1/checkout/sessions',{method:'POST',headers:{Authorization:`Bearer ${this.secretKey}`,'Content-Type':'application/x-www-form-urlencoded'},body:form.toString()}),body=await response.json() as {id?:string;url?:string;error?:{message?:string}};if(!response.ok||!body.id||!body.url)throw new Error(body.error?.message||'Could not create a secure checkout session.');return{id:body.id,url:body.url}}
