...
import { GatewayStore, type GatewayAccount } from './store.ts';
...
export class GatewayService{
 constructor(private readonly store:GatewayStore,private readonly providerKeys:ProviderKeys,private readonly fetcher:FetchLike=fetch,private readonly billing?:BillingService,private readonly productionReadiness=false){}
 async handle(request:GatewayRequest):Promise<GatewayResponse>{try{
  ...
  if(request.method==='POST'&&request.path==='/v1/billing/webhook')return this.billingWebhook(request);
  const account=this.authorize(request.headers);
  ...
  if(request.method==='GET'&&request.path==='/v1/billing/history')return{status:200,body:{purchases:this.store.purchases(account.id)}};
 ...
 private authorize(headers:Record<string,string|undefined>|undefined):GatewayAccount{const value=headers?.authorization??headers?.Authorization;if(!value?.startsWith('Bearer '))throw new Error('Authentication required.');return this.store.authenticate(value.slice(7))}
