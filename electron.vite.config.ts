...
export default defineConfig({
  main:{plugins:[externalizeDepsPlugin()],build:{rollupOptions:{external:['electron-builder'],output:{format:'cjs',entryFileNames:'[name].cjs'}}}},preload:{plugins:[externalizeDepsPlugin()],build:{rollupOptions:{input:{index:resolve('src/preload/index.ts'),runtime:resolve('src/preload/runtime.ts')},output:{format:'cjs',entryFileNames:'[name].cjs'}}}},
