...
- The unpacked `Foundry.exe` and native DLLs are signed before packaging. Electron-builder's Azure integration signs the generated NSIS uninstaller, and the final installer is signed separately.
- Signed release gates perform a clean installation and require valid Authenticode chains, RFC 3161 timestamps, and an exact expected certificate subject on `Foundry.exe`, `Uninstall Foundry.exe`, and the installer before publication.
